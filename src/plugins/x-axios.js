...
export default {
  install: (app, options) => {
    ...
    service.interceptors.response.use(
      (response) => {
        return interceptors?.response?.(response) || response
      },
      (error) => {
        return Promise.reject(interceptors?.responseError?.(error) || error)
      }
    )
    ...
    // 注册全局xaxios
    app.prototype.$xaxios = $xaxios

    // 注册别名axios
    if (alias) {
      app.prototype[alias] = $xaxios
    }
  }
}
